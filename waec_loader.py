...
def get_subject_folder(subject_code: str) -> Path:
    ...
    Example: chem -> data/waec/chemistry/
    """
...
def get_course_subject_map() -> List[Dict[str, Any]]:
    ...
    data/waec/course_subject_map.json
    """
    file_path = WAEC_DATA_DIR / "course_subject_map.json"
...
def validate_course_subject_map() -> List[str]:
    ...
    active_subject_codes = {subject["code"] for subject in get_waec_subjects()}
...
def prepare_use_of_english_batch(
    ...
    """
    Prepare a WAEC-structured English paper using the fixed blueprint.
    ...
    for section in ENG_EXACT_BLUEPRINT:
        ...
        for question in picked_questions:
            question["_waec_section"] = section_name
...
def format_course_subjects_for_message(course_code: str) -> str:
    ...
    Useful for Mock WAEC/NECO course recommendation screens.
    """
    course = get_course_by_code(course_code)
    if not course:
...
def get_waec_subject_topics(subject_code: str) -> List[Dict[str, Any]]:
    return get_subject_topics(subject_code)
...
def prepare_waec_topic_question_batch(
    ...
    return prepare_topic_question_batch(
        subject_code=subject_code,
        topic_id=topic_id,
        requested_count=requested_count,
        seen_question_ids=seen_question_ids,
...
def prepare_waec_subject_question_batch(
    subject_code: str,
    requested_count: int,
    seen_question_ids: List[str],
    start_topic_index: int = 0,
) -> Dict[str, Any]:
    return prepare_subject_question_batch(
        subject_code=subject_code,
        requested_count=requested_count,
        seen_question_ids=seen_question_ids,
        start_topic_index=start_topic_index,
...
def prepare_waec_english_objective_batch(
    seen_question_ids: List[str],
) -> Dict[str, Any]:
    return prepare_use_of_english_batch(
        seen_question_ids=seen_question_ids,
    )
