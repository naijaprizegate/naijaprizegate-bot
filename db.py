...
#   2) PgBouncer transaction pooler prepared-statement errors
#      (disable asyncpg statement cache)
#
# Notes:
#   - We REMOVE sslmode/pgbouncer params from URL because asyncpg
#     doesn't accept them
#   - We REQUIRE TLS but SKIP cert-chain verification
...
def _sanitize_asyncpg_url(url: str) -> str:
    ...
    Supabase pooler URLs may include parameters such as:
      - sslmode=require
      - pgbouncer=true
      - pool_mode=...
      - ssl=...

    We remove those parameters from the URL and handle SSL through
...
# This is equivalent to libpq sslmode=require behavior.
...
# asyncpg prepared-statement caching to be disabled.
#
# DO NOT use prepared_statement_name_func here.
# -------------------------------------------------
...
async def init_db():
    ...
    logger.info(
        "✅ Database initialized (development use only)"
    )
...
async def init_game_state():
    async with async_sessionmaker() as session:
        ...
        logger.info(
            "🎯 init_game_state: ensured baseline game data"
        )
