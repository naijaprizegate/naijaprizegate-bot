# ======================================================
# services/mockwaec_session_service.py
# ======================================================
import json
import logging
...
async def create_mockwaec_session(
    session: AsyncSession,
    *,
    payment_reference: str,
    user_id: int,
    ...
    existing = await get_mockwaec_session_by_payment_reference(session, payment_reference)
    if existing:
        return existing

    await session.execute(
        text("""
            insert into public.mockwaec_sessions (
                payment_reference,
                user_id,
                course_code,
                subject_codes_json,
...
async def get_or_create_mockwaec_session_from_payment(
    session: AsyncSession,
    *,
    payment_reference: str,
    user_id: int,
    ...
    return await create_mockwaec_session(
        session,
        payment_reference=payment_reference,
        user_id=user_id,
        course_code=course_code,
...
async def start_mockwaec_session_if_needed(
    session: AsyncSession,
    *,
    payment_reference: str,
) -> dict | None:
...
async def set_mockwaec_current_subject(
    session: AsyncSession,
    *,
    payment_reference: str,
    subject_code: str,
...
async def mark_mockwaec_subject_completed(
    session: AsyncSession,
    *,
    payment_reference: str,
    subject_code: str,
...
async def get_seen_mockwaec_question_ids(
    session: AsyncSession,
    *,
    user_id: int,
    subject_code: str,
    ...
    result = await session.execute(
        text("""
            select question_id
            from public.mockwaec_seen_questions
            where user_id = :user_id
              and subject_code = :subject_code
              and source_type = 'mockwaec'
            order by id asc
        """),
        {
            "user_id": int(user_id),
...
async def record_seen_mockwaec_questions(
    session: AsyncSession,
    *,
    user_id: int,
    subject_code: str,
    ...
    for question_id in question_ids:
        await session.execute(
            text("""
                insert into public.mockwaec_seen_questions (
                    user_id,
                    subject_code,
                    question_id,
                    source_type,
...
async def get_latest_active_mockwaec_session_for_user(
    session: AsyncSession,
    *,
    user_id: int,
) -> dict | None:
