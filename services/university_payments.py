# =======================================================
# services/university_payments.py
# =======================================================
...
logger = logging.getLogger("university_payments")
logger.setLevel(logging.INFO)


async def get_university_payment(
...
async def create_pending_university_payment(
    session: AsyncSession,
    *,
    payment_reference: str,
    user_id: int,
    ...
    existing = await get_university_payment(
        session,
        payment_reference,
    )
    ...
    if credits <= 0 and mock_sessions <= 0:
        raise ValueError(
            f"Invalid UNIVERSITY payment package for amount {amount_paid}"
        )

    await session.execute(
        text("""
            insert into university_payments (
                payment_reference,
                user_id,
                amount_paid,
                question_credits_added,
    ...
    return await get_university_payment(
        session,
        payment_reference,
    )


async def finalize_university_payment(
    session: AsyncSession,
    *,
    payment_reference: str,
    user_id: int,
    ...
    """
    Safe/idempotent UNIVERSITY finalizer.
    ...
    payment = await get_university_payment(
        session,
        payment_reference,
    )
    ...
    if not payment:
        ...
        payment = await create_pending_university_payment(
            session,
            payment_reference=payment_reference,
            user_id=int(user_id),
            amount_paid=int(amount_paid),
    ...
    credits = int(
        payment.get("question_credits_added")
        or question_credits_added
        or 0
    )

    mock_sessions = int(
        payment.get("mock_sessions_added")
        or mock_sessions_added
        or 0
    )
    ...
    if credits <= 0 and mock_sessions <= 0:
        ...
        logger.error(
            "❌ Invalid UNIVERSITY package | payment_reference=%s | amount=%s",
            payment_reference,
            amount_paid,
        )
    ...
    if not claimed_row:
        ...
        latest = await get_university_payment(
            session,
            payment_reference,
        )
    ...
    await session.execute(
        text("""
            insert into university_user_access (
                user_id
            )
            values (
                :user_id
            ...
            text("""
                ...
                set
                    mock_sessions_available =
                        mock_sessions_available + :mock_sessions,
